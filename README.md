...
Так как мы хотим получить хоть каплю удовольствия от написаня кода и будем использовать для этого [lua](https://www.lua.org/), то не будем изобретать лишние слова и создадим папку **lua** в корне папки с игрой.

```
mkdir "D:\Warcraft III\lua"
```

Так как ребята из Майкрософт не знают, что джентельменам принято верить на слово, то астрологи предсказывают ложные срабатывания анивируса. Чтоб избежать сей участи, добавьте только что созданную папку в исключения антивируса.
